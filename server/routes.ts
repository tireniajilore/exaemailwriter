...
const DEPLOY_VERSION = "2025-12-28-replit";
const PROMPT_VERSION = "v9.0-voice-first";
const MODEL_NAME = "google/gemini-2.5-flash";
const EXA_RESEARCH_TIMEOUT_MS = 180000;
const EXA_RESEARCH_POLL_INTERVAL_MS = 3000;
...
IDENTITY DECISION:
- PASS_HIGH (confidence >= 0.75): Full name + company confirmed
- PASS_LOW (confidence 0.45-0.74): Partial match, company confirmed
- FAIL (confidence < 0.45): Cannot reliably identify person

EVIDENCE REQUIREMENTS (GATE 2 - WHAT COUNTS AS A HOOK)
A valid hook MUST include at least one:
✓ QUOTE - Exact words they said
✓ NAMED_INITIATIVE - A specific named project/program they created
✓ DESCRIBED_DECISION - A specific choice they made
✓ NAMED_ARTIFACT - A specific talk, article, book, paper
...
async function createExaResearchTask(
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error(`[exa_research] Create task failed: status=${response.status} body=${errorText}`);
      return { researchId: "", error: `Exa API error: ${response.status}` };
    }

    const data = await response.json();
    console.log(`[exa_research] Task created: researchId=${data.researchId}`);
    return { researchId: data.researchId };
  } catch (e) {
    console.error(`[exa_research] Create task error: ${e}`);
...
async function pollExaResearchTask(
...
): Promise<{ researchId: string; status: string; output?: FlatExaResearchOutput; error?: string; latency_ms?: number }> {
  ...
  while (Date.now() - startTime < timeoutMs) {
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error(`[exa_research] Poll failed: status=${response.status}`);
        return {
          researchId,
          status: "failed",
      ...
      const data = await response.json();
      console.log(`[exa_research] Poll ${pollCount}: status=${data.status}`);

      if (data.status === "completed") {
        const latency = Date.now() - startTime;
        console.log(`[exa_research] Completed in ${latency}ms (${pollCount} polls)`);
      ...
      console.error(`[exa_research] Poll error: ${e}`);
    }
  }
...
async function performV2Research(
  ...
  if (createError || !researchId) {
    console.error(`[exa_research] Failed to create task: ${createError}`);
    return {
      hookPacks: [],
  ...
  if (result.status === "failed" || !result.output) {
    console.error(`[exa_research] Task failed: ${result.error}`);
    return {
      hookPacks: [],
      senderIntentProfile: intentProfile,
      minimalResearch: true,
  ...
  console.log(`[exa_research] identity_decision=${output.identity_decision} hook_packs=${output.hook_packs.length}`);
...
export async function registerRoutes(app: Express): Promise<void> {
  app.post("/api/prolific/session", async (req, res) => {
    try {
      const { prolificId, studyId, prolificSessionId, profession, coldEmailFrequency } = req.body;
      ...
      if (!isTestId && !isValidProlificFormat) {
        if (trimmedProlificId.length < 3) {
          return res.status(400).json({ error: 'Prolific ID must be at least 3 characters' });
        }
      }
      ...
      if (!coldEmailFrequency || typeof coldEmailFrequency !== 'string') {
        return res.status(400).json({ error: 'Cold email frequency is required' });
      }

      const existingSession = await storage.getProlificSessionByProlificId(trimmedProlificId);
      if (existingSession) {
        return res.json({ sessionId: existingSession.id });
      }

      const session = await storage.createProlificSession({
        prolificId: trimmedProlificId,
        studyId: studyId?.trim() || null,
        prolificSessionId: prolificSessionId?.trim() || null,
  ...
  app.post("/api/prolific/survey", async (req, res) => {
    try {
      const { 
        sessionId,
        comparisonRating,
      ...
      if (comparisonRating !== undefined && (typeof comparisonRating !== 'number' || comparisonRating < 1 || comparisonRating > 5)) {
        return res.status(400).json({ error: 'Comparison rating must be between 1 and 5' });
      }
      ...
      if (likelihoodChange && !validLikelihoodChanges.includes(likelihoodChange)) {
        return res.status(400).json({ error: 'Invalid likelihood change value' });
      }

      const session = await storage.getProlificSessionById(sessionId);
      if (!session) {
        return res.status(400).json({ error: 'Invalid session' });
      }

      if (session.completedAt) {
        return res.status(409).json({ error: 'Survey has already been submitted for this session' });
      }

      const survey = await storage.createProlificPostSurvey({
        sessionId,
        comparisonRating,
        likelihoodChange,
        likelihoodReasons: likelihoodReasons || null,
        changesBeforeSending: changesBeforeSending || null,
        whatFeltOff: whatFeltOff?.substring(0, 5000) || null,
        mostUsefulPart: mostUsefulPart || null,
        whatsMissing: whatsMissing?.substring(0, 5000) || null,
      });

      await storage.updateProlificSessionCompletedAt(sessionId, new Date());

      res.json({ surveyId: survey.id, success: true });
    } catch (error) {
  ...
  app.post("/api/generate-email", async (req, res) => {
    ...
    try {
      ...
      console.log(`BOOT generate-email deploy_version=${DEPLOY_VERSION} generation_id=${generationId}`);
      ...
      if (!recipientRole || recipientRole.length < 1 || recipientRole.length > 100) {
        return res.status(400).json({ error: "Invalid role/title" });
      }

      if (!credibilityStory || credibilityStory.length < 10 || credibilityStory.length > 1000) {
        return res.status(400).json({ error: "Credibility story must be between 10 and 1000 characters" });
      }

      if (!reachingOutBecause || reachingOutBecause.length < 5 || reachingOutBecause.length > 500) {
        return res.status(400).json({ error: "Please explain why you are reaching out (5-500 characters)" });
      }
      ...
      if (EXA_API_KEY) {
        ...
        try {
          ...
          console.log(`V2 Research complete: ${researchResult.hookPacks.length} Hook Packs`);
        } catch (e) {
...
Return JSON only:
{
  ...
  "body": "..."
      ...
      try {
        ...
        return res.status(500).json({ error: "Failed to generate valid email. Please try again." });
      }
      ...
      try {
        await storage.createEmailGeneration({
          sessionId: sessionId,
          source,
          ...
          researchModelName: "exa-research",
          subject: emailData.subject,
          body: emailData.body,
          wordCount: wordCount,
          clicheCount: clicheCount,
          likeYouCount: likeYouCount,
          hasEmDash: hasEmDash(emailData.body),
          ...
          exaResults: researchResult?.citations.map((c) => ({ url: c.url, title: c.title })) || [],
          selectedSources: researchResult?.citations.map((c) => c.url) || [],
          researchedFacts: researchResult?.hookPacks.map((hp) => hp.hook_fact.claim) || [],
          latencyMs: latencyMs,
        });
      } catch (logError) {
